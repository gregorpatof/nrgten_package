 ...
 **********************************************************************/
 ...
 // This program was modified by Olivier Mailhot in 2020 to make it usable with Python
 // bindings using ctypes. This makes it easy to include in the ENCoM Python package.
 // Added/modified functions are all commented accordingly.
...
int mock_calc_region(struct atom PDB[], int PDBtot, int atomlist[], int numatoms, int atom_nums[], char atom_names[],
                     int resi_nums[], char resi_names[], char chains[], float areas[], float dists[]); // added by Olivier
...
int run_from_python(char filedata[], int atom_nums[], char atom_names[], int resi_nums[], char resi_names[],
    ...
    mock_calc_region(PDB, PDBtot, PDBlist, PDBtot, atom_nums, atom_names, resi_nums, resi_names,
                     chains, areas, dists);
    return(0);
}
...
int mock_main(char filedata[]) { // duplicated by Olivier
    int      atomi;           // atom number of atomzero
    long int PDBtot;          // number of PDB atoms
...
int mock_calc_region(struct atom PDB[], int PDBtot, int atomlist[], int numatoms, int atom_nums[], char atom_names[],
    ...
    for(atomi=0; atomi<PDBtot; ++atomi) {
        ...
        contnum = 1;
//        printf("%5d | %5s | %4d | %5s | %1c | Sol_acc_surf | %.9g\n",

        base_index = from_atom_count*100;
        to_atom_count = 1;

        atom_nums[base_index] = PDB[atomi].atomnum;
	    strncpy(&atom_names[base_index], PDB[atomi].atomname, 5);
	    resi_nums[base_index] = PDB[atomi].resnum;
	    strncpy(&resi_names[base_index], PDB[atomi].res, 4);
	    chains[base_index] = PDB[atomi].chain;
	    areas[base_index] = -1.0;
	    dists[base_index] = (normalize=='Y')?(PDB[atomi].SAS*100.0/(rado*rado*4.0*PI)):PDB[atomi].SAS;

        while(currindex != -1) {
            // normalize if required
            if(normalize == 'Y') { // Olivier : never happens but could be interesting to test
	            ca_rec[currindex].area *= (100.0/(rado*rado*4.0*PI));
            }
            // Remark by Olivier : showbonded is no longer taken into account. Covalently bonded contacts are not reported.
            // To change this behavior, remove the below condition.
...
//                    printf("%5d | %5s | %4d | %5s | %1c | %.9g | %.9g\n",

                    base_index = from_atom_count*100 + to_atom_count;

                    atom_nums[base_index] = PDB[ca_rec[currindex].atom].atomnum;
                    strncpy(&atom_names[base_index*5],PDB[ca_rec[currindex].atom].atomname, 5);
                    resi_nums[base_index] = PDB[ca_rec[currindex].atom].resnum;
                    strncpy(&resi_names[base_index*4], PDB[ca_rec[currindex].atom].res, 4);
                    chains[base_index] = PDB[atomi].chain;
                    areas[base_index] = ca_rec[currindex].area;
                    dists[base_index] = ca_rec[currindex].dist;
//                    printf("to_atom_count: %s\n", to_atom_count);
                    ++to_atom_count;
...
struct atom *mock_read_PDB(char filedata[], struct atom *PDB, long int *numPDBatoms) { // duplicated by Olivier
    char oneline[89];
    char *curLine = filedata;
    long int PDBtot;
    long int PDBarraysize;

//    printf("started mock_read_pdb!\n");
    ...
    while (1) {
        ...
        strcpy(oneline, curLine);
        // ########### End
        if ((strncmp(oneline, "END",3)==0) || (curLine == NULL)) {
            *numPDBatoms = PDBtot;
